...
_alpha = [0.005, 0.01, .03, .05, .07, 0.1]
                ...
                for conc_ss_id, conc_ss in enumerate(_concept_sigmoid_spacing):
                    ...
                    for arch_id, arch in enumerate(_architectures):
                        for a_id, a in enumerate(_alpha):
                            ...
                            det = CDET(base_mlp=base, alpha=a, n_epochs=150)
                            ...
                            for chunk_id in range(_n_chunks):
                                ...
                                det.process(X, y)
                                if det.is_drift:
                                    res_dets[r_id, ch_s_id, n_f_id, conc_ss_id, arch_id, a_id, chunk_id] = 1
                        
                            # exit()
                            pbar.update(1)
                        print(np.sum(res_dets[r_id, ch_s_id, n_f_id, conc_ss_id, arch_id], axis=1))

                    np.save('res/exp0_cdet.npy', res_dets)
