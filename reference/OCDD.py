# https://github.com/ogozuacik/one-class-drift-detection/blob/master/OCDD.py

import numpy as np
from sklearn import svm
class OneClassDriftDetector():
    def __init__(self, size, dim, percent, nu):
        self.size = size
        self.dim = dim
    ...
    def process(self, X):
        ...
        for instance in X:
            ...
            if self.first == True:
                self.clf.fit(instance.reshape(1,-1))
                self.first = False
    ...
    def addInstance(self, instance):
        if(self.isEmpty()):
            self.win_data[self.window_index] = instance
            self.win_outlier[self.window_index] = self.clf.predict(instance)
